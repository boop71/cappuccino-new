...
function Material.new(title, style, x, y)
	local Config = {}
	local Style = style and math.clamp(style, 1, 3) or 1
	local Title = tostring(title) or "MaterialLua"
	local SizeX = x or 300
	local SizeY = y or 500
	...
	function TabLibrary:Page(title)
		local TabConfig = {Title = tostring(title)}
		...
		function OptionLibrary:Button(ButtonConfig)
			ButtonConfig.Text = tostring(ButtonConfig.text)
		...
		function OptionLibrary:Dropdown(DropdownConfig)
			...
			local DropdownOptions = DropdownConfig.values or {}
		...
		function OptionLibrary:Toggle(ToggleConfig)
			...
			local ToggleDefault = ToggleConfig.state or false
		...
		function OptionLibrary:Keybind(kC)
			...
			kC.key = typeof(kC.key) == 'EnumItem' and kC.key or typeof(kC.key) == 'string' and (function()
				local d,a = xpcall(function()
					return Enum.KeyCode[kC.key]
				end,function()
					return nil
				end)
				return a
		...
		function OptionLibrary:Textbox(TextFieldConfig)
		...
		function OptionLibrary:Slider(SliderConfig)
			...
			local SliderDef = math.clamp(SliderConfig.value, SliderMin, SliderMax) or math.clamp(50, SliderMin, SliderMax)
			...
			local Rounding = Instance.new('UICorner', SliderTracker)
